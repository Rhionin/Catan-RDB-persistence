...
var RDBGameStore = (function() {
	...
	*/
	...
	RDBGameStore.prototype.addGame = function(gameJson, callback) {
		...
		that.rdb.insert(this.tableName, ["data"], [JSON.stringify(gameJson)], function(id){
			gameJson.id = id;
			that.updateGame(gameJson, function(changes){
				if(callback)
					callback(id);
			});
		});
	...
	*/
    ...
    @method getGame
    @param {int} id
    @param {int} getOriginal
    
	*/
	RDBGameStore.prototype.getGame = function(id, getOriginal, callback) {


		if(arguments.length == 2)
		{
			callback = getOriginal;
			getOriginal = true;	
		}
		...
		{
			rdb.select(tableName, {id:id}, function(row){
				if(row[0])
	...
	*/
	RDBGameStore.prototype.initialize = function(callback) 
	{
		...
		this.rdb.select(this.tableName, {}, function(rows){
			...
			for(var i=0; i<rows.length; ++i)
			{
				var gameId = models[i].id;
				var index = i;
				that.getGame(gameId, false, function(gameJson){
					models[index] = gameJson;
					if(--unloadedGames == 0)
						callback(models);
				});
			}
