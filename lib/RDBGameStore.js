...
var RDBGameStore = (function() {
	...
	*/
	RDBGameStore.prototype.updateGame = function(gameJson, gameId, callback) {	
		
		this.rdb.update(this.tableName, {id:gameId}, {data:JSON.stringify(gameJson)}, function(changes){
			if(callback)
				callback(changes);
		});
	...
	*/
	RDBGameStore.prototype.addCheckpoint = function(gameId, gameJson, callback) {
		...
		this.rdb.insert(this.checkpointTable, ["game-id", "data"], [gameId, JSON.stringify(gameJson)], function(id){
			if(callback)
				callback(id);
		});
